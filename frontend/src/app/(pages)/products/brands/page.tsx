...
import { Tag, Plus, Pencil, Eye, Trash2, Upload, FileDown, FileText, SlidersHorizontal } from "lucide-react";
...
function exportBrandsCsv(brands: ProductBrand[]) {
  ...
  const csvText = [header, ...rows]
    ...
    .join("\n");
  downloadBlob(new Blob([csvText], { type: "text/csv;charset=utf-8;" }), `brands_${exportDateStamp()}.csv`);
}
...
export default function BrandsPage() {
  ...
  const [viewBrand,        setViewBrand]        = useState<ProductBrand | null>(null);
  const [confirmBrand,     setConfirmBrand]     = useState<ProductBrand | null>(null);
  const [importOpen,       setImportOpen]       = useState(false);
  const [selectedKeys,     setSelectedKeys]     = useState<Set<string>>(new Set());
  const [allPagesSelected, setAllPagesSelected] = useState(false);

  const { data: brands = [], isLoading } = useQuery<ProductBrand[]>({
    queryKey: ["brands"],
  ...
  const filteredBrands = useMemo(() => {
    ...
    const filtered = brands.filter((b) => {
      const matchesSearch =
        ...
        (b.manufacturer_name?.toLowerCase().includes(q) ?? false);
      const matchesStatus =
        statusFilter === ""      ? true :
        statusFilter === "true"  ? b.is_active :
                                   !b.is_active;
  ...
  }, [brands, search, statusFilter, sort]);
  ...
  const currentPageKeys     = pagedBrands.map((b) => b.id);
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: (brand: ProductBrand) =>
      apiPatch<ProductBrand>(`/products/brands/${brand.id}`, { is_active: !brand.is_active }),
    onSuccess: (_, brand) => {
  ...
  async function handleDownloadTemplate(): Promise<void> {
    const blob = await apiDownloadFile("/products/brands/import/template");
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-0.5">
          <button
            title="View Details"
            onClick={(e) => { e.stopPropagation(); setViewBrand(row); }}
          ...
          {canManage && (
            <>
              <button
                title="Edit Brand"
                onClick={(e) => { e.stopPropagation(); setEditingBrand(row); setModalOpen(true); }}
                className="p-1.5 rounded-md transition-colors hover:bg-[var(--color-surface-2)]"
                style={{ color: "var(--color-text-muted)" }}
              >
              ...
              <button
                ...
                onClick={(e) => { e.stopPropagation(); setConfirmBrand(row); }}
                className={
                  row.is_active
  ...
  const isDeactivating = confirmBrand?.is_active === true;
  ...
  return (
    <div className="page-container">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <Tag className="w-6 h-6" style={{ color: "var(--color-text-muted)" }} />
            <h1 className="page-title">Brands</h1>
          </div>
          <p className="page-subtitle mt-1">
            Manage pharmaceutical brands and their manufacturers
          </p>
        </div>
          ...
          {canManage && (
            ...
            >
              ...
              <Button
                ...
                onClick={() => { setEditingBrand(null); setModalOpen(true); }}
              >
                New Brand
              </Button>
            </div>
      ...
      <BrandModal
        isOpen={modalOpen}
        onClose={() => { setModalOpen(false); setEditingBrand(null); }}
      ...
      <ConfirmModal
        ...
        title={isDeactivating ? "Deactivate Brand" : "Activate Brand"}
        body={
          isDeactivating ? (
            <>
        ...
        onConfirm={() => confirmBrand && toggleStatusMutation.mutate(confirmBrand)}
