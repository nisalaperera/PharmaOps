...
interface GenericQuickCreateModalProps {
  isOpen:       boolean;
  onClose:      () => void;
  initialName?: string;
...
export function GenericQuickCreateModal({
  isOpen, onClose, initialName = "", onCreated,
}: GenericQuickCreateModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: (data: FormValues) => apiPost<ProductGeneric>("/products/generics", data),
    onSuccess: (created) => {
      queryClient.invalidateQueries({ queryKey: ["generics"] });
      showToast("success", "Generic Created", `${created.name} has been added.`);
