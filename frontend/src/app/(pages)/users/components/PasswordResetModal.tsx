...
import { passwordResetSchema, type PasswordResetValues } from "@/app/(pages)/users/schemas";
...
export function PasswordResetModal({ isOpen, onClose, user }: PasswordResetModalProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast("success", "Password Reset", `${user!.full_name}'s password has been updated successfully.`);
      onClose();
    },
    onError: (err: { message?: string }) => {
      showToast("error", "Reset Failed", err?.message ?? "Something went wrong. Please try again.");
    },
  });
