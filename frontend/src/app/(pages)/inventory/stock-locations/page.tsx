...
import { MapPin, Plus, Pencil, Eye, Trash2, Upload, FileDown, FileText, SlidersHorizontal } from "lucide-react";
...
import type { StockLocation, ImportResult } from "@/types";

// ─── Export helpers ──────────────────────────────────────────────────────────

function buildRow(loc: StockLocation): string[] {
  return [loc.name, loc.code, loc.description ?? "", loc.is_active ? "Active" : "Inactive"];
}
...
function exportCsv(items: StockLocation[]) {
  const header  = ["Name", "Code", "Description", "Status"];
  const rows    = items.map(buildRow);
  const csvText = [header, ...rows]
    .map((row) => row.map((c) => `"${c.replace(/"/g, '""')}"`).join(","))
    .join("\n");
  downloadBlob(new Blob([csvText], { type: "text/csv;charset=utf-8;" }), `stock_locations_${exportDateStamp()}.csv`);
}

async function exportPdf(items: StockLocation[]) {
  const doc     = new jsPDF();
  ...
  doc.text(`Stock Locations — ${exportDateStamp()}`, 14, cursorY + 4);
  cursorY += 10;

  autoTable(doc, { head: headers, body, startY: cursorY, styles: { fontSize: 8 } });
  doc.save(`stock_locations_${exportDateStamp()}.pdf`);
}
...
export default function StockLocationsPage() {
  ...
  const [viewLocation,     setViewLocation]     = useState<StockLocation | null>(null);
  const [confirmLocation,  setConfirmLocation]  = useState<StockLocation | null>(null);
  ...
  const filteredLocations = useMemo(() => {
    ...
    const filtered = locations.filter((loc) => {
      ...
      const matchesStatus =
        statusFilter === ""     ? true :
        statusFilter === "true" ? loc.is_active :
                                  !loc.is_active;
  ...
  const currentPageKeys     = pagedLocations.map((l) => l.id);
  ...
  const selectedItems  = filteredLocations.filter((l) => selectedKeys.has(l.id));
  const selectionCount = allPagesSelected ? totalItems : selectedKeys.size;

  function handleExportCsv() {
    exportCsv(allPagesSelected ? filteredLocations : selectedItems);
  }

  async function handleExportPdf() {
    await exportPdf(selectedItems);
  }

  const toggleStatusMutation = useMutation({
    mutationFn: (loc: StockLocation) =>
      apiPatch<StockLocation>(`/stock-locations/${loc.id}`, { is_active: !loc.is_active }),
    onSuccess: (_, loc) => {
      queryClient.invalidateQueries({ queryKey: ["stock-locations"] });
      showToast(
        "success",
        loc.is_active ? "Location Deactivated" : "Location Activated",
        loc.is_active ? `${loc.name} has been deactivated.` : `${loc.name} is now active.`,
      );
  ...
  async function handleImport(file: File): Promise<ImportResult> {
    const result = await apiUploadFile<ImportResult>("/stock-locations/import", file);
    queryClient.invalidateQueries({ queryKey: ["stock-locations"] });
    return result;
  }

  async function handleDownloadTemplate(): Promise<void> {
    const blob = await apiDownloadFile("/stock-locations/import/template");
    downloadBlob(blob, "stock_locations_import_template.csv");
  }
  ...
  const columns: Column<StockLocation>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <p className="text-sm truncate max-w-[300px]" style={{ color: "var(--color-text-muted)" }}>
          {row.description || "—"}
        </p>
      ),
    },
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-0.5">
          <button title="View Details" onClick={(e) => { e.stopPropagation(); setViewLocation(row); }}
          ...
          {canManage && (
            <>
              <button title="Edit Location" onClick={(e) => { e.stopPropagation(); setEditingLocation(row); setModalOpen(true); }}
                className="p-1.5 rounded-md transition-colors hover:bg-[var(--color-surface-2)]"
                style={{ color: "var(--color-text-muted)" }}>
                <Pencil className="w-3.5 h-3.5" />
              </button>
              <button title={row.is_active ? "Deactivate" : "Activate"}
                onClick={(e) => { e.stopPropagation(); setConfirmLocation(row); }}
  ...
  const isDeactivating = confirmLocation?.is_active === true;
  ...
  return (
    <div className="page-container">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <MapPin className="w-6 h-6" style={{ color: "var(--color-text-muted)" }} />
            <h1 className="page-title">Stock Locations</h1>
          </div>
          <p className="page-subtitle mt-1">Manage shelves, racks, and storage areas for this branch</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <Button variant={filterVisible || hasActiveFilters ? "primary" : "outline"} size="sm"
          ...
          <SearchBar placeholder="Search locations…" onSearch={handleSearch} className="w-[22rem] max-w-full" />
          {canManage && (
            <div className="flex items-center gap-2 pl-3 ml-1 border-l flex-shrink-0" style={{ borderColor: "var(--color-border)" }}>
              <Button variant="primary" leftIcon={<Upload className="w-4 h-4" />} onClick={() => setImportOpen(true)}>Import</Button>
              <Button variant="primary" leftIcon={<Plus className="w-4 h-4" />}
                onClick={() => { setEditingLocation(null); setModalOpen(true); }}>New Location</Button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl shadow-card overflow-hidden" style={{ background: "var(--color-surface)" }}>
        {showSelectAllBanner && (
          <div className="px-4 py-2 text-sm text-center border-b" style={{ background: "var(--color-surface-2)", borderColor: "var(--color-border)" }}>
            ...
            <button onClick={handleSelectAllPages} className="font-semibold text-primary-500 hover:underline">Select all {totalItems} records</button>
          </div>
        )}
        {allPagesSelected && (
          <div className="px-4 py-2 text-sm text-center border-b" style={{ background: "var(--color-surface-2)", borderColor: "var(--color-border)" }}>
        ...
        <DataTable<StockLocation>
          columns={columns} data={pagedLocations} isLoading={isLoading}
          rowKey={(row) => row.id} sort={sort} onSort={handleSort}
          emptyMessage={search || hasActiveFilters ? "No locations found matching the current filters." : "No stock locations found. Add one to get started."}
        ...
        {(selectedKeys.size > 0 || allPagesSelected) && (
          <div className="border-t flex items-center justify-between px-4 py-3 gap-3"
            style={{ borderColor: "var(--color-border)", background: "var(--color-surface-2)" }}>
            <p className="text-sm font-medium" style={{ color: "var(--color-text)" }}>
              {selectionCount} record{selectionCount !== 1 ? "s" : ""} selected
            </p>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" leftIcon={<FileDown className="w-3.5 h-3.5" />} onClick={handleExportCsv}>Export CSV</Button>
              {!allPagesSelected && (
                <Button variant="outline" size="sm" leftIcon={<FileText className="w-3.5 h-3.5" />} onClick={handleExportPdf}>Export PDF</Button>
              )}
              <button onClick={clearSelection} className="text-xs px-2 py-1 rounded transition-colors hover:bg-[var(--color-surface)]"
      ...
      <StockLocationModal isOpen={modalOpen} onClose={() => { setModalOpen(false); setEditingLocation(null); }} editingLocation={editingLocation} />
      <StockLocationViewModal isOpen={!!viewLocation} onClose={() => setViewLocation(null)} location={viewLocation} />
      <ImportModal isOpen={importOpen} onClose={() => setImportOpen(false)} entityName="Stock Locations"
        onImport={handleImport} onDownloadTemplate={handleDownloadTemplate}
        templateNote="Required: name, code. Optional: description, is_active (TRUE/FALSE, defaults to TRUE)." />
      <ConfirmModal
        isOpen={!!confirmLocation} onClose={() => setConfirmLocation(null)}
        title={isDeactivating ? "Deactivate Location" : "Activate Location"}
        body={isDeactivating ? (
          <>Are you sure you want to deactivate <span className="font-semibold" style={{ color: "var(--color-text)" }}>{confirmLocation?.name}</span>?</>
        ) : (
          <>Are you sure you want to activate <span className="font-semibold" style={{ color: "var(--color-text)" }}>{confirmLocation?.name}</span>?</>
        )}
        confirmLabel={isDeactivating ? "Deactivate" : "Activate"}
        variant={isDeactivating ? "danger" : "primary"}
        onConfirm={() => confirmLocation && toggleStatusMutation.mutate(confirmLocation)}
