...
import { PAYMENT_METHOD_VARIANT, SALE_STATUS_VARIANT } from "@/lib/badges";
...
export function SaleViewModal({ sale, onClose }: SaleViewModalProps) {
  if (!sale) return null;
  ...
  async function handlePrintPdf() {
    ...
    const summaryRows: string[][] = [
      ...
      ["Payment",  PAYMENT_METHOD_LABEL[s.payment_method] ?? s.payment_method],
    ...
    if (s.status !== "COMPLETED") {
      summaryRows.push(["Status", SALE_STATUS_LABEL[s.status] ?? s.status]);
      if (s.refund_amount && s.refund_amount > 0) {
        summaryRows.push(["Refunded", s.refund_amount.toFixed(2)]);
      }
    }
  ...
  return (
    <Modal
      ...
      size="md"
      headerExtra={
        <Badge variant={SALE_STATUS_VARIANT[sale.status] ?? "default"} dot>
    ...
    >
      <div className="space-y-5">
        ...
        <div className="rounded-xl px-4 py-3 space-y-1.5" style={{ background: "var(--color-surface-2)" }}>
          ...
          {sale.refund_amount != null && sale.refund_amount > 0 && (
            <div className="flex justify-between text-sm">
              <span style={{ color: "var(--color-text-muted)" }}>Refunded</span>
              <span className="tabular-nums font-semibold text-danger-500">{formatAmount(sale.refund_amount)}</span>
            </div>
          )}
        </div>
