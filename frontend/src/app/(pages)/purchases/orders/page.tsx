...
export default function PurchaseOrdersPage() {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<PurchaseOrder>>({
    ...
    queryFn:  () => apiGet<PaginatedResponse<PurchaseOrder>>("/purchases/orders", { ...queryParams, ...filters }),
  ...
  const actionMutation = useMutation({
    mutationFn: ({ po, type }: { po: PurchaseOrder; type: POAction }) =>
      apiPost<PurchaseOrder>(`/purchases/orders/${po.id}/${type}`),
    onSuccess: (_, { po, type }) => {
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canManage && (
            <div className="flex items-center gap-2 pl-3 ml-1 border-l flex-shrink-0" style={{ borderColor: "var(--color-border)" }}>
              <Button variant="primary" leftIcon={<Plus className="w-4 h-4" />} onClick={() => { setEditingPO(null); setModalOpen(true); }}>
                New Order
              </Button>
            </div>
          )}
